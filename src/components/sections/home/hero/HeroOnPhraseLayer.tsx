...
type HeroPhraseLayerProps = {
  ...
  onCopyVisible?: () => void;
  onActiveShapeChange?: (
    shape: "all" | "circle" | "triangle" | "square",
  ) => void;
...
const PhraseLine = ({
  ...
  <div
    ...
    style={{
      ...
      textShadow: '0 0 16px rgba(255,255,255,1), 0 0 32px rgba(255,255,255,0.8)',
    }}
  >
    {children}
  </div>
...
export default function HeroOnPhraseLayer({
  ...
  const lineVisible = [sequenceStep >= 1, sequenceStep >= 2, sequenceStep >= 3];
  ...
  return (
    <div
      className={cn(
        ...
        "items-center gap-[0.24em] translate-x-0 -translate-y-[5%]",
    ...
    >
      ...
      >
        <PhraseLine isOn={isOn} visible={lineVisible[0]} baseColor={baseColor}>
          <span
            onMouseEnter={() =>
              !isTouch && sequenceStep >= 4 && onActiveShapeChange?.("circle")
            }
        ...
        <PhraseLine isOn={isOn} visible={lineVisible[1]} baseColor={baseColor}>
          <span
            onMouseEnter={() =>
              !isTouch && sequenceStep >= 4 && onActiveShapeChange?.("triangle")
            }
            onMouseLeave={() => !isTouch && onActiveShapeChange?.("all")}
            onTouchStart={() =>
              isTouch && sequenceStep >= 4 && onActiveShapeChange?.("triangle")
        ...
        <PhraseLine isOn={isOn} visible={lineVisible[2]} baseColor={baseColor}>
          ...
          <span
            id="hero-nemo-origin"
            onMouseEnter={() =>
              !isTouch && sequenceStep >= 4 && onActiveShapeChange?.("square")
            }
            onMouseLeave={() => !isTouch && onActiveShapeChange?.("all")}
            onTouchStart={() =>
              isTouch && sequenceStep >= 4 && onActiveShapeChange?.("square")

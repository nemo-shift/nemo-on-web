...
export default function OfferingsStudio() {
  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto w-full flex flex-col gap-5 tablet-p:gap-8">
          ...
          <div className="flex flex-col gap-6 studio-caps">
            ...
            <div className="flex flex-wrap gap-2 tablet-p:gap-3">
              {data.works.map((work, idx) => (
                <div
                  key={idx}
                  className="px-2.5 py-1 tablet-p:px-4 tablet-p:py-2 rounded-md tablet-p:rounded-lg bg-white/5 border border-white/10 text-[11px] tablet-p:text-sm font-light text-gray-200 transition-all hover:bg-white/10 studio-cap-tag"
                >
                  {work}
                </div>
              ))}

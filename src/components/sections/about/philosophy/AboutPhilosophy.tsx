...
export default function AboutPhilosophy() {
  ...
  const labelRef = useRef<HTMLDivElement>(null); // 🆕 메타 라벨 제어용 ref 추가
  ...
  useGSAP(() => {
    ...
    tl.to(phase2Ref.current, {
      ...
      duration: 0.07,
      ease: 'power2.out'
    }, 0.53); // 🆕 1번이 사라진 후 빈 공간 대기하다가 53% 지점부터 2번이 진입 시작

    // 5. [Phase 2 정독 구간 극대화] 60%부터 98% 지점까지 스크롤 내리는 내내 완전 고정 (지속시간 0.38)
    tl.to({}, { duration: 0.38 }, 0.6); // 🆕 38% 휠 거리에 달하는 훨씬 더 넓은 정독 존 확보!

    // 6. [오버레이 지연 마감 대기] 98%부터 100% 지점까지 비로소 다음 카드를 위해 대기 (지속시간 0.02)
    tl.to({}, { duration: 0.02 }, 0.98); // 🆕 다음 오버레이 타이밍을 98% 최하단으로 연기하여 극지연!

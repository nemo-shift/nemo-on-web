...
11. **[디자인 토큰 중앙화]**: 모든 UI/애니메이션 상수는 `src/constants/interaction.ts`에서 관리하며, 코드 내 하드코딩을 금지한다. (v16.38 추가)
12. **[조건부 시각 보고]**: 주요 레이아웃의 첫 구현, 대규모 변경, 또는 사용자 요청 시에만 `browser` 도구를 통해 녹화/캡처 보고를 수행하여 효율을 높인다. (v16.39 추가)

...
- `getComputedStyle`로 `translate`는 변하는데 `scale`만 멈추었는지 확인:
  ...
  const style = window.getComputedStyle(
    document.querySelector(".nemo-logo-engine"),
  );
...
> [!NOTE]
> **[V11 구조 변경]** 아래 Final Spec의 `scale` 기반 엔진은 현재 폐기됨.
> Dual-Axis 이중 제동 시스템 도입 시 `min(vw, vh)` 수식이 `scale` 속성에 적용 불가하여
> `font-size` 기반 엔진으로 전환. 현행 표준은 `globals.css`의 `.nemo-logo-engine` 참조.

...
# [확장] STEP 2-B — 섹션 내부 오브젝트 위치의 뷰포트 의존 제거

> 배경: STEP 1~2는 섹션 높이와 finalY(전체 스크롤 거리)를 svh로 고정하지만,
> 섹션 "내부" 오브젝트들의 위치가 별도로 뷰포트에 의존하고 있어 크롬 상태에
> 따라 어긋난다. 원인은 두 가지: (A) GSAP 트윈의 'Nvh' 문자열 값 — GSAP은
> 이를 px로 변환할 때 그 시점의 window.innerHeight를 사용하므로 빌드 시점
> 뷰포트가 박제된다. (B) 빌더 내 명시적 window.innerHeight 계산.

## 2-B-1. stableVH를 빌더에 주입

STEP 2에서 만든 `stableVH` 값을 빌더 옵션 객체(builderOptions 또는 각 빌더의
파라미터)에 `stableVH: number`로 추가해 모든 빌더가 접근할 수 있게 한다.
공용 헬퍼도 추가한다 (builders/index.ts 또는 global-interaction-utils.ts):

```ts
// [V67.ViewportFix] 'Nvh' 상당의 안정 px 값 (svh 기준)
export const svhPx = (n: number, stableVH: number) => (n / 100) * stableVH;
```

## 2-B-2. (원인 A) 빌더의 'Nvh' 문자열 값을 stableVH 기반 px로 교체

대상 파일과 지점 (grep으로 재확인 후 전수 교체):

- `builders/story.ts` — 문단 1~5의 x/y 포지션 전부
  (예: `y: '-14vh'` → `y: svhPx(-14, stableVH)`; `'25vw'` 등 vw 값은 그대로 둠.
  `parseFloat(p1Pos.y) + 5 + 'vh'` 같은 문자열 연산은
  `svhPx(-14 + 5, stableVH)` 형태의 숫자 연산으로 재작성)
- `builders/message.ts` — `y: '120vh'`, `y: '-120vh'` → `svhPx(±120, stableVH)`
- `builders/funnel.ts` — 모바일/태블릿 `y: '-8vh'`, `'-7vh'` → svhPx 교체
- 그 외 `grep -rn "vh'" components/sections/home/builders` 결과 전부.

주의: GSAP 트윈 값을 문자열('...vh')에서 숫자(px)로 바꾸면 단위 해석이
달라지므로, fromTo의 from/to 양쪽을 모두 같은 방식으로 교체해야 한다.

## 2-B-3. (원인 B) 빌더의 명시적 innerHeight 교체

- `builders/forwho.ts` 약 87행:
  `if (str.includes('vh')) return window.innerHeight * (parseFloat(str)/100);`
  → `return svhPx(parseFloat(str), stableVH);`
- `builders/nemo.ts` 약 70~77행: `const vh = window.innerHeight;` →
  `const vh = stableVH;` (이후 % 재계산 로직은 그대로 유지 — 기준값만 교체)

## 2-B-4. 히어로 뷰의 CSS vh 인라인 스타일을 svh로 교체

CSS는 GSAP과 달리 단위를 브라우저가 직접 해석하므로 단순 단위 교체로 충분:

- `hero/HeroSection.tsx`: `height: isMobileView ? '60vh' : '90vh'` → `'60svh'/'90svh'`
- `hero/views/HeroOffPCView.tsx`: `height: '80vh'`, `translateY(15vh)`,
  `bottom: '-20vh'`, `gap-[1vh]` → 전부 svh
- `hero/views/HeroOffMobileView.tsx`: `gap-[6vh]`, `marginTop: '-7vh'`,
  `bottom: '-28vh'` → svh
- `hero/views/HeroOffTabletView.tsx`: `gap-[4vh]`, `marginTop: '-1vh'` → svh
- `GlobalScrollHint.tsx`: `clamp(24px, 4vh, 48px)` → `clamp(24px, 4svh, 48px)`
- `pain/PainSection.tsx`: 스크롤 트랙 `h-[1000vh]` → `h-[1000svh]`
  (offsetHeight로 실측되므로 기능상 어느 쪽이든 동작하지만, 기준 단위 통일을
  위해 교체. 교체 후 Pain 구간 체감 스크롤 길이가 미세하게 짧아질 수 있으니
  검증 단계에서 확인)

## 2-B-5. 검증 추가 항목

- `grep -rn "innerHeight" components/sections/home/builders` 결과가 0건인지
  (또는 남은 건에 [V67.ViewportFix-검토필요] 주석과 사유가 있는지)
- `grep -rn "vh'" components/sections/home/builders` 결과가 0건인지
- 실기기 테스트에 추가: **페이지 중간(주소창 접힌 상태)에서 새로고침** 후
  스토리/메시지/포후 섹션의 오브젝트 위치가, 최상단(주소창 펼침)에서
  새로고침했을 때와 동일한지 비교. 두 경우가 같아야 수정 성공.

## 하지 말 것 (확장분)

- `vw` 단위는 건드리지 않는다 (가로폭은 크롬과 무관).
- FallingKeywordsStage / useParticles의 캔버스 innerHeight는 여전히 제외.
- 히어로 뷰 파일에 이미 있는 '100svh' 표기는 그대로 유지.
